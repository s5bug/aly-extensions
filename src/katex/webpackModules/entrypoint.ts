...
const KaTeXElement = (props: { katexCode: string; displayMode: boolean }) => {
  ...
  React.useEffect(() => {
    // biome-ignore lint/style/noNonNullAssertion: we are guaranteed to have a container element
...
const inlineMarkdownParse: ParseFunction = (capture, _nestedParse, _state) => {
  return {
    content: capture[0],
    katexCode: capture[1],
  }
...
const inlineSlateRule = (_rules: Record<string, SlateRule>): SlateRule => {
...
const blockMarkdownParse: ParseFunction = (capture, _nestedParse, _state) => {
  return {
    content: capture[0],
    katexCode: capture[1],
  }
...
const blockSlateRule = (_rules: Record<string, SlateRule>): SlateRule => {
  return { type: 'verbatim' }
}
