...
const rubySegmentMatch: MatchFunction = ((regex) => {
  const f: MatchFunction = (source, _state, _prevCapture) => {
...
const rubySegmentParse: ParseFunction = (
  capture,
  nestedParse,
  state,
): RubyASTNode => {
  ...
  const rubyGroups: RubyGroup[] = []
  let match: RegExpMatchArray | null
  // biome-ignore lint/suspicious/noAssignInExpressions: an assignment expression is the natural way to express this
...
const rubyRenderer: SingleNodeOutput<React.ReactNode> = (
  { rubyGroups, leftovers }: RubyASTNode | SingleASTNode,
  nestedOutput,
  state,
) => {
  const groupElems: React.ReactNode[] = rubyGroups.flatMap(
    ({ base, ruby }: RubyGroup) => {
      return rubyGroupRenderer(
        base.map((v) => nestedOutput(v, state)),
        ruby.map((v) => nestedOutput(v, state)),
      )
    },
  )
  const leftoverElems: React.ReactNode[] = leftovers.map((v: SingleASTNode) =>
    nestedOutput(v, state),
  )
...
const rubySlateRule = (_rules: Record<string, SlateRule>): SlateRule => {
  return { type: 'verbatim' }
}

markdown.addRule('ruby', rubyMarkdownRule, rubySlateRule)
